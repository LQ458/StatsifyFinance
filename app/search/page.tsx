...
const Search = () => {
  ...
  const addMark = (content: string, keyword: string) => {
    ...
    if (content.indexOf(keyword) > -1) {
      ...
      result = content.replace(
        regex,
        `<span style="color:#FFD700">${keyword}</span>`,
      );
    }
    return result;
  };
  ...
  const getArticles = async (key: string) => {
    ...
    list?.data?.list &&
      list.data.list.map((item: Article) => {
        filterList.push({
          ...item,
  ...
  const getFinanceTerms = async (key: string) => {
    ...
    list?.data?.list &&
      list.data.list.map((item: FinanceTerms) => {
        filterList.push({
          ...item,
          title: addMark(item.title, key),
          ...
          <div className={`${styles.newsContent}`}>
            <div className={`${mainNav.nav} w-full`}>
              <ul className="border-b border-[#666] ">
                <li
                  className={`${currentNav === "articles" ? mainNav.active : ""}`}
                >
                  <Link href="#">资讯</Link>
                </li>
            ...
            <div className=" pt-[20px] text-[#fff] text-[14px]">
               找到与“<span className="text-[#FFD700]">{keywords}</span>
              ”相关的内容共{articlesList.length}条
            </div>
